...
us_tuition %>%
  ggplot(aes(year, cost, group = state)) +
  geom_line() +
  geom_smooth(method = "lm", group = 1)

```

That just tells us that the states are all rising in cost over time. I suspect that some states are rising at different rates. One way to see this clearer would be to create a simple linear model for each state that compares year and cost. Then sort the results by the highest coefficient. 
